...
def download_video(av,target_dir=r""):
    """ 解析命令行参数并调用相关模块进行下载 """

    if (sys.version_info.major, sys.version_info.minor) < (3, 8):
        print("请使用 Python3.8 及以上版本哦～")
        sys.exit(1)
    config_url=base_url+av       # 视频主页地址
    cofig_type="dash"   #choices=["flv", "dash", "mp4"]
    ...
    config_audio=30216   # [30280, 30232, 30216] 音频码率等级 30280:320kbps, 30232:128kbps, 30216:64kbps
    config_playlist="dpl"   # ["dpl", "m3u", "no"] 播放列表类型，支持 dpl 和 m3u，输入 no 不生成播放列表
    config_danmaku="xml"   # ["xml", "ass", "no"] 弹幕类型，支持 xml 和 ass，如果设置为 no 则不下载弹幕
    config_block=128   #  分块下载器的块大小，单位为 MB，默认为 128MB，设置为 0 时禁用分块下载
    config_abs="store_true"   # 修改播放列表路径类型为绝对路径
    config_mirrors="store_true"   # 启用从多个镜像下载功能
    config_proxy="store_true"   # 禁用系统代理
    config_debug="store_true"   # debug模式

    cookies = {"SESSDATA": config_cookies}

    config = {
        "url": config_url,
        "dir": config_dir,
        "quality": config_quality,
        "audio_quality": config_audio,
        "episodes": config_episodes,
        "playlist_type": config_playlist,
        "playlist_path_type": "AP" if config_abs else "RP",
        "overwrite": config_overwrite,
        "cookies": cookies,
        "type": cofig_type.lower(),
        "block_size": int(config_block * 1024 * 1024),
    } >> AttrDict()
    ...
    # fmt: off
    if (avid_match := regex.acg_video.av.origin.match(config_url)) or \
        (avid_match := regex.acg_video.av.short.match(config_url)):
    ...
    elif (bvid_match := regex.acg_video.bv.origin.match(config_url)) or \
        (bvid_match := regex.acg_video.bv.short.match(config_url)):
        from bilili.api.acg_video import get_video_info
    ...
    elif media_id_match := regex.bangumi.md.origin.match(config_url):
    ...
    elif (episode_id_match := regex.bangumi.ep.origin.match(config_url)) or \
        (episode_id_match := regex.bangumi.ep.short.match(config_url)):
        episode_id = episode_id_match.group("episode_id")
        resource_id.episode_id = episode_id
    elif (season_id_match := regex.bangumi.ss.origin.match(config_url)) or \
        (season_id_match := regex.bangumi.ss.short.match(config_url)):
    ...
    # 获取需要的信息
    containers = [BililiContainer(video_dir=video_dir, type=cofig_type, **video) for video in get_list(resource_id)]
    ...
    if playlist is not None:
        playlist.flush()
    # 写出url文件
    with open( os.path.join(video_dir, "details.txt"), 'w') as f:
        f.write("url:"+config_url)
    # 准备下载
    if containers:
        # 状态检查与校正
        for i, container in enumerate(containers):
            container_downloaded = not container.check_needs_download(config_overwrite)
            symbol = "✓" if container_downloaded else "✖"
            if container_downloaded:
                container._.merged = True
            print("{} {}".format(symbol, str(container)))
            for media in container.medias:
                media_downloaded = not media.check_needs_download(config_overwrite) or container_downloaded
                symbol = "✓" if media_downloaded else "✖"
                if not container_downloaded:
                    print("    {} {}".format(symbol, media.name))
                for block in media.blocks:
                    block_downloaded = not block.check_needs_download(config_overwrite) or media_downloaded
                    symbol = "✓" if block_downloaded else "✖"
                    block._.downloaded = block_downloaded
                    if not media_downloaded and config_debug:
        ...
        download_pool = ThreadPool(config_threads, daemon=True, thread_globals_creator={
        ...
        for container in containers:
            ...
            for media in container.medias:
                ...
                for block in media.blocks:
                    ...
                    mirrors = block.mirrors if config_mirrors else []
        ...
        # 初始化界面
        console = Console(debug=config_debug)
